...
- **single 的拐点 = 色度天花板**：12M→40M 字节 +76% 只换 +2.1dB（曲线
  明显变平），B 通道被 4:2:0 亚采样压死在 36dB 一线。
- **ycocg 无天花板**：40M 档冲到 45dB 近透明（三通道均衡 44.3~45.9），
  曲线后段反而更陡（QP 钳制解除后每字节收益兑现）。
- **QP 钳制台阶**（单帧 IDR 实测，流式多帧不存在）：
  - single：2M→45KB ｜ [3M,5M]→52KB ｜ 6M→91KB ｜ 12M→271KB ｜ 40M→478KB
  - ycocg：[1.5M,4M]→74KB ｜ [5M,9M]→84KB ｜ 12M→143KB ｜ 20M→274KB ｜ 40M→794KB

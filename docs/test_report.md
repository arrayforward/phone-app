...
### 3.2 新桌面（亮绿草地+蓝天，彩色高频内容，26 档全表）

| 路径 | 标称码率 | 产出字节 | PSNR 总 | R / G / B |
|---|---|---|---|---|
| single | 2M | 45 KB | 27.19 | 26.23 / 28.05 / 27.50 |
| single | 3M | 52 KB | 28.35 | 27.53 / 29.21 / 28.46 |
| single | 4M | 52 KB | 28.35 | 27.53 / 29.21 / 28.46 |
| single | 5M | 52 KB | 28.35 | 27.53 / 29.21 / 28.46 |
| single | 6M | 91 KB | 30.90 | 30.30 / 32.40 / 30.30 |
| single | 12M | 271 KB | 36.44 | 36.37 / 39.56 / 34.69 |
| single | 24M | 478 KB | 38.57 | 38.94 / 42.87 / 36.24 |
| single | 30M | 478 KB | 38.57 | 38.94 / 42.87 / 36.24 |
| single | 40M | 478 KB | 38.57 | 38.94 / 42.87 / 36.24 |
| single | 100M | 478 KB | 38.57 | 38.94 / 42.87 / 36.24 |
...
| dual-ycocg | 1.5M | 74 KB | 28.17 | 27.37 / 28.06 / 29.29 |
| dual-ycocg | 2M | 74 KB | 28.17 | 27.37 / 28.06 / 29.29 |
| dual-ycocg | 2.5M | 74 KB | 28.17 | 27.37 / 28.06 / 29.29 |
| dual-ycocg | 4M | 74 KB | 28.17 | 27.37 / 28.06 / 29.29 |
| dual-ycocg | 5M | 84 KB | 29.58 | 28.87 / 29.42 / 30.65 |
...
| dual-ycocg | 24M | 452 KB | 40.34 | 40.14 / 40.64 / 40.24 |
| dual-ycocg | 30M | 452 KB | 40.34 | 40.14 / 40.64 / 40.24 |
| dual-ycocg | 40M | 794 KB | **44.98** | 44.86 / 45.94 / 44.29 |
| dual-ycocg | 100M | 794 KB | **44.98** | 44.86 / 45.94 / 44.29 |
...
- **高码率档 YCoCg 优势最大**：40M 档 QP 钳制彻底解除，ycocg 达
  **45dB 级近透明质量**（三通道 44.3~45.9 均衡），single 受 4:2:0 结构性
  ...
  它没有。
- **中高码率最干净的一组对决**：ycocg@24M/30M（452KB，40.34dB）用更少
  5.4% 字节反超 single@24M/30M（478KB，38.57dB）+1.77dB——single 在
  24M 已封顶，ycocg 还在爬坡。
...
![RD 曲线](images/rd_curve.png)

PSNR vs 实际码流字节（log-x，同一编码器同内容，单帧 IDR）：

- **拐点 = 同字节交叉点 ~272KB / 36.5dB**：ycocg@20M 与 single@12M 实际
  字节差 <1%，ycocg 反超 0.21dB。拐点左侧（低预算）single 占优（双 YUV
  的 2× 像素开销是净负担）；拐点右侧 ycocg 占优且差距随预算扩大。
- **single 的拐点 = 色度天花板**：12M→24M 字节 +76% 只换 +2.1dB，且
  **[24M,∞) 完全封顶**（24M/30M/40M/100M 逐字节相同 478KB / 38.57dB）——
  B 通道被 4:2:0 亚采样压死在 36dB 一线，加码率无用。
- **ycocg 无天花板**：40M 档冲到 45dB 近透明（三通道均衡 44.3~45.9）；
  中高码率段 ycocg 持续爬坡（20M→[24M,30M]→40M 三跳 36.65→40.34→44.98）。
- **QP 钳制台阶**（单帧 IDR 实测，流式多帧不存在）：
  - single：2M→45KB ｜ [3M,5M]→52KB ｜ 6M→91KB ｜ 12M→271KB ｜ **[24M,∞)→478KB（封顶）**
  - ycocg：[1.5M,4M]→74KB ｜ [5M,9M]→84KB ｜ 12M→143KB ｜ 20M→274KB ｜
    [24M,30M]→452KB ｜ **[40M,∞)→794KB（封顶）**
  - 生产按台阶对齐 `--bitrate` 才有意义（台阶内的标称码率不生效）。
- **dual-raw（§3.1）**：各档均不占优（同画质字节最多），已被 §3.2 取代，
  仅作对照保留。

**生产建议**：btl/码率充裕段（>~10Mbps 标称）用 YCoCg 双 YUV 拿画质上限；
低预算段（<~6M 标称）切 `--yuv-raw` 或降分辨率更划算。
